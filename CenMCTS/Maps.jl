function add_map(model, map, nb_robots)
    D = 2
    f = open("/home/mathilde/Documents/These/Codes/SimulateurCentralizedMCTS/maps/map$map.txt", "r")
    id = nb_robots+1
    _ = readline(f)
    triche_map = parse(Int64, readline(f))
    for line in readlines(f)
        str_pos = split(line, "\t")
        pos = (parse(Int64, str_pos[1]),parse(Int64, str_pos[2]))
        agent = Robot{D}(id, pos, 0, 0, true)
...
function add_obstacles(model; N = 5, extent = (20,20))
    D = length(extent)
    n_robot = length(collect(allagents(model)))
...
function add_simple_obstacles(model, extent; N = 1, min_obstacle_size = 7, max_obstacle_size = 10)
    count = 0
    attempts = 0
    ids = length(collect(allagents(model)))
        ...
        if is_valid_position(model, extent, x, y, obstacle_width, obstacle_height)
            ...
            for i in x:x+obstacle_width
                for j in y:y+obstacle_height
                    ids += 1
                    agent = Robot{2}(ids, (i,j), 0, 0, true)
...
function create_map(size)
    ...
    X = [0,size[1]*50,size[1]*50,0,0]
    Y = [0,0,size[2]*50,size[2]*50,0]

    lines!(scene, X,Y, color = :red, linewidth=50)
    ...
    on(events(scene).mousebutton) do event
        if event.button == Mouse.left && event.action == Mouse.press
            ...
            global x2,y2 = floor(Int64, mp[1]/50), floor(Int64, mp[2]/50)
            if first_press == true
                first_press = false
                x1,y1 = x2,y2
                ...
                lines!(scene, [x1*50,x2*50], [y1*50,y2*50], color = :black, linewidth=50)
                first_press = true
    ...
    on(events(scene).mouseposition) do mp
        ...
        println("x = $(floor(Int64, mp[1]/50)), y = $(floor(Int64, mp[2]/50))")
    end
