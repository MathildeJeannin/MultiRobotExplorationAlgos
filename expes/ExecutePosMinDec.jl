...
try
    mkdir("Resultats/DecPositionMinimum"*folder)
catch e
end
try
    mkdir("Logs/DecPositionMinimum"*folder)
catch e
end


nb_steps, cov = run(vis_figure = false, nb_robots=nb_robots, max_steps=max_steps, num_map=num_map, file="Logs/DecPositionMinimum"*folder, id_expe=N, extent=(extent1,extent2), nb_blocs=nb_blocs, begin_zone=(begin_zone,begin_zone))
...
CSV.write("Resultats/DecPositionMinimum"*file, df, writeheader=true, delim = ';', append=true)

log_file = open("Logs/DecPositionMinimum"*folder*"$N.txt", "a")
