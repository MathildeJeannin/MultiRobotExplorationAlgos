...
function nearby_obstacles(pos::Tuple, model::ABM, r::Int)
    return filter(obj->(obj.pos in nearby_positions(pos,model,r) && typeof(obj) == Obstacle{2}), [model[i] for i in (abmproperties(model).nb_robots+1):(abmproperties(model).nb_obstacles+abmproperties(model).nb_robots)])
end
